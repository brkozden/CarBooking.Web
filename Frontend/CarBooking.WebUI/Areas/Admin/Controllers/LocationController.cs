...
namespace CarBooking.WebUI.Areas.Admin.Controllers
{
    ...
    public class LocationController : Controller
    {
        ...
        public async Task<IActionResult> CreateLocation(CreateLocationDto createLocation)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7057/api/Locations", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7057/api/Locations/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Location", new { area = "Admin" });
        ...
        public async Task<IActionResult> UpdateLocation(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7057/api/Locations/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateLocation(UpdateLocationDto updateLocation)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7057/api/Locations/", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Location", new { area = "Admin" });
