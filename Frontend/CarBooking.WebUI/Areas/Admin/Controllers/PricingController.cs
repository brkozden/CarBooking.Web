using CarBooking.Dtos.PricingDtos;
...
namespace CarBooking.WebUI.Areas.Admin.Controllers
{
    ...
    public class PricingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7057/api/Pricings");
        ...
        public async Task<IActionResult> CreatePricing(CreatePricingDto createPricing)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7057/api/Pricings", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7057/api/Pricings/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Pricing", new { area = "Admin" });
        ...
        public async Task<IActionResult> UpdatePricing(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7057/api/Pricings/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdatePricing(UpdatePricingDto updatePricing)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7057/api/Pricings/", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Pricing", new { area = "Admin" });
