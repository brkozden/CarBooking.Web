using CarBooking.Dtos.AuthorDtos;
...
namespace CarBooking.WebUI.Areas.Admin.Controllers
{
    ...
    public class AuthorController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7057/api/Authors");
        ...
        public async Task<IActionResult> CreateAuthor(CreateAuthorDto createAuthor)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7057/api/Authors", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7057/api/Authors/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Author", new { area = "Admin" });
        ...
        public async Task<IActionResult> UpdateAuthor(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7057/api/Authors/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateAuthor(UpdateAuthorDto updateAuthor)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7057/api/Authors/", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "Author", new { area = "Admin" });
